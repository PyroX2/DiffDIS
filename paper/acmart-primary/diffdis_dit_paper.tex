...
Dichotomous image segmentation (DIS) is the task of producing a high-accuracy binary foreground mask for arbitrary objects in natural images. Unlike class-specific semantic segmentation, DIS is category agnostic. Unlike coarse salient object detection, it emphasizes accurate object silhouettes, thin structures, holes, and high-frequency details. This makes the task difficult: a successful model needs a wide receptive field to understand global object extent, but it must also preserve local detail at boundaries and internal structures.
...
This is a conservative initialization: it avoids randomizing the first projection completely, but it also assumes that the pre-trained projection can be meaningfully reused for the new channel layout. The results suggest that this assumption is only partially valid.
...
\subsection{Datasets}
We report two evaluation settings. The first is the original DIS5K setting, including DIS-VD and DIS-TE1 through DIS-TE4. The second is an additional smaller dataset, marked as ``New dataset'' in our result sheets, with DIS-VD and DIS-TE1 measurements. The additional dataset is useful as a secondary sanity check, but it is smaller and should not be interpreted as a replacement for the full DIS5K benchmark.
...
Table~\ref{tab:new-results} reports the additional smaller dataset. The modified model is still weaker, but the gap is less uniform. On the additional DIS-VD subset, reproduced DiffDIS obtains maxF $0.7575$ and the modified model obtains $0.5457$. On the additional DIS-TE1 subset, reproduced DiffDIS obtains maxF $0.9326$, while the modified model obtains $0.5217$. The result suggests that the modified model can learn some foreground structure, but not with the precision needed for DIS.

\begin{table}[t]
\centering
\caption{Results on the additional smaller dataset.}
